...
import (
	...
	"resource_admin_service/docs" // docs is generated by Swag CLI, you have to import it.
	...
	"resource_admin_service/handlers"
	"resource_admin_service/repository"
	"resource_admin_service/usecase"
)
...
func main() {
	...
	kafkaService := models.NewKafka(NewKafkaProducer(), eventChan, tools.InitLogger("./logs/kafka-logs.txt", "kafka:"))
	go kafkaService.ExecuteEvents()

	repository := repository.NewRepositoryGORM(InitDBGORM())
	usecase := usecase.NewUseCase(repository, eventChan, tools.InitLogger("./logs/logs.txt", "usecase: "))
